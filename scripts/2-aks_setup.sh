...
helm --debug install csi csi-secrets-store-provider-azure/csi-secrets-store-provider-azure -n csi
...
helm install pod-identity aad-pod-identity/aad-pod-identity -n aad-pod-id

# Install specific version from tar.gz instead of "latest"
# helm --debug install pod-identity https://github.com/Azure/aad-pod-identity/blob/master/charts/aad-pod-identity-2.0.3.tgz -n aad-pod-id
